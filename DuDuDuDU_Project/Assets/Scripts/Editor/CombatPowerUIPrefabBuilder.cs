...
using OJ.CombatPower;

namespace OJ.EditorTools
{
    // Editor-only authoring. Gameplay code never creates this UI hierarchy.
    //
    // 자동 실행 금지 (MIGRATION_BASELINE 1.1). 예전에는 [InitializeOnLoad] + delayCall 로
    // 에디터를 켤 때마다 LobbyScene 을 열어 프리팹을 심고 저장했다. 그래서 씬을 손보는
    // 작업이 다음 에디터 재기동에 조용히 덮어써졌다. 이제 메뉴로만 실행된다.
