    ...
    public sealed class TowerRunManager
    {
        ...
        public static List<PointRewardEntry> BuildClearRewards(int floor, bool firstClear)
        {
            ...
            if (TowerFormula.IsBandLastFloor(floor))
            {
                ...
                // <b>전투 강화석을 주면 안 된다.</b> 그것은 판이 시작될 때 0 으로 밀리는
                // 인게임 재화라(RunState.BeginRun), 받자마자 다음 층에 들어가면 사라지고
                // 로비에는 쓸 곳조차 없다 — 주는 척만 하는 보상이었다.
                //
                // 지금은 이 실수가 <b>컴파일되지 않는다</b> — 그 재화는 PointType 이 아니라
                // BattlePointType 이고, 이 목록은 PointRewardEntry(영구 재화)만 받는다.
                //
                ...
                rewards.Add(new PointRewardEntry(
                    PointType.MythicStone, TowerFormula.BandRewardMaterial(floor)));
            }
